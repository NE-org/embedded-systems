...
# Added the unisex Images for the Sunglasses
sunglasses_unisex = [
    cv2.imread(f'images/unisex{i}.png', cv2.IMREAD_UNCHANGED) for i in range(1, 33)
]

# Ensure all unisex images are loaded correctly
sunglasses_unisex = [img for img in sunglasses_unisex if img is not None]
...
def detect_blink(landmarks, blink_threshold=5.0):
    ...
    if left_eye_height < blink_threshold and right_eye_height < blink_threshold:
        return True
    return False
...
def main():
    ...
    ser = serial.Serial('/dev/cu.usbmodem2017_2_251', 9600)  # Replace 'COM12' with your actual serial port
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            face_parameters = (x, y, w, h)  # Store the face parameters globally
            ...
            if sunglasses_resized.shape[2] == 4:  # Ensure image has alpha channel
                ...
                print("Error: Sunglasses image does not have an alpha channel")

        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results_hands = hands.process(rgb_frame)
        results_face_mesh = face_mesh.process(rgb_frame)
        
        ok_sign_detected = detect_ok_sign(rgb_frame, results_hands.multi_hand_landmarks)
        ...
        if results_face_mesh.multi_face_landmarks:
            for face_landmarks in results_face_mesh.multi_face_landmarks:
                if detect_blink(face_landmarks.landmark):
                    blink_detected = not blink_detected  # Toggle between primary and unisex sunglasses
                    break
        
        if blink_detected and face_parameters:
            x, y, w, h = face_parameters  # Retrieve the stored face parameters
            current_sunglasses = np.random.choice(sunglasses_unisex)
            if current_sunglasses is not None:
                sunglasses_name = 'Unisex Sunglasses'
                # Calculate the position and size of the sunglasses
                sunglasses_width = int(sunglasses_scale * w)
                ...
                if sunglasses_resized.shape[2] == 4:  # Ensure image has alpha channel
                    ...
                    frame_roi = frame[y1:y2, x1:x2]
                     ...
                     # Resize sunglasses mask to match frame_roi if necessary
                    if sunglasses_mask.shape[:2] != frame_roi.shape[:2]:
                        sunglasses_mask = cv2.resize(sunglasses_mask, (frame_roi.shape[1], frame_roi.shape[0]))

                    # Blend the sunglasses with the frame
                    for c in range(0, 3):
                        if frame_roi.shape == sunglasses_resized.shape[:2]:
                            frame_roi[:, :, c] = (1.0 - sunglasses_mask) * frame_roi[:, :, c] + sunglasses_mask * sunglasses_resized[:, :, c]
                        else:
                            print("Error: Sunglasses image size does not match frame_roi size")
                else:
                    print("Error: Sunglasses image does not have an alpha channel")
            else:
                print("Error: current_sunglasses is None")
